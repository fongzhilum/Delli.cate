...
// ── Junk filter ───────────────────────────────────────────────
const JUNK = [
  /afrikaans|arabic|deutsch|français|español|italiano|português|русский|中文|日本語|한국어|bahasa|svenska|norsk|polski|tiếng/i,
  /^(following|followers|posts|more|next|back|close|mute|pause|play|share|reply|seen by|send message)$/i,
  /meta about blog jobs help api privacy terms/i,
  /instagram from meta/i,
  /^[\d,]+$/,   // pure numbers
  /^.{1}$/,     // single char
];
...
// ── Scrape only the story overlay ────────────────────────────
function scrapeStoryText() {
  const texts = [];
  const seen  = new Set();

  // Walk every leaf text node on the page
  const walker = document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT);
  let node;
  while ((node = walker.nextNode())) {
    const t = node.textContent?.trim();
    if (!t || t.length < 3 || t.length > 280) continue;
    if (seen.has(t)) continue;
    if (isJunk(t)) continue;

    // Only include if the element (or an ancestor) is positioned over the video
    // i.e. has z-index or is inside a known story overlay container
    ...
    // Skip Instagram's profile/bio area (only present outside stories)
    if (el.closest("section > div > div > div > div > div > div > div > header")) continue;
...
async function scan() {
  ...
  console.log(`[Delli.cate] text (${text.length} chars): "${text.slice(0, 120)}"`);
